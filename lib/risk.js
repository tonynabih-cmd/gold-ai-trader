...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    // ── RULE 5: Dynamic High-Liquidity Window Model ────────────────────────
    // Defines 3 session states for XAUUSD to gate trades based on market volume.
    const min = now.getUTCMinutes();
    const timeFloat = hour + min / 60;
    
    let sessionMode = 'LOW_LIQUIDITY';

    // HIGH LIQUIDITY MODE: London open (07:00-10:00 UTC), NY open/Overlap (12:30-16:30 UTC)
    if ((timeFloat >= 7 && timeFloat < 10) || (timeFloat >= 12.5 && timeFloat <= 16.5)) {
      sessionMode = 'HIGH_LIQUIDITY';
    }
    // NORMAL MODE: Mid-session/lunch (10:00-12:30 UTC), Late NY (16:30-19:00 UTC)
    else if ((timeFloat >= 10 && timeFloat < 12.5) || (timeFloat > 16.5 && timeFloat <= 19.0)) {
      sessionMode = 'NORMAL';
    }
    // LOW LIQUIDITY MODE: Asia session + Dead hours (19:00-07:00 UTC)
    else {
      sessionMode = 'LOW_LIQUIDITY';
    }

    // Evaluate gating based on session mode:
    if (sessionMode === 'LOW_LIQUIDITY') {
      // Proxy for "extremely clear Layer 1 trend regime"
      // Require EMA20 steepness to show undeniable commitment during quiet hours.
      const slopeAbs = typeof indicators.slopePercent === 'number' ? Math.abs(indicators.slopePercent) : 0;
      if (slopeAbs < 0.15) {
        return `SKIP: Low liquidity session (${hour}:${min.toString().padStart(2, '0')} UTC) - Layer 1 trend not extremely clear (slope=${slopeAbs.toFixed(3)})`;
      }
    }
    ...
    // ── RULE 10b: Sideways Market Filter — REMOVED ────────────────────────────
    // Strategy.js has its own trend establishment check (ATR*0.35 normal, ATR*0.20 relaxed)
    // that is context-aware and sufficient. This duplicate filter at ATR*0.4 was
    // creating a dead zone where strategy approved signals but risk blocked them.
    ...
    // Realized loss must not exceed 5% of account balance.
    ...
    // ── RULE 22: Margin buffer check (Risk check - uses EQUITY/AVAILABLE) ──────
    {
      ...
      if (!isNaN(availableMargin) && availableMargin > 0) {
        ...
        const targetRiskPct    = 0.02; // 2.0% target risk per trade
