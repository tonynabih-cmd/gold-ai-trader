...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    // FIX: Changed from permanent block to 30-minute cooldown.
    // The old logic created an unrecoverable deadlock: 2 losses → can't trade → can't win
    // → anti-chop never clears. Now it cools down for 30min then allows recovery.
    ...
    if (recentLosses.length >= 2) {
      const lastLossTime = Math.max(...recentLosses.map(o => o.closedAt || 0));
      if (lastLossTime > 0 && (Date.now() - lastLossTime) < 30 * 60 * 1000) {
        ...
        return `SKIP: Anti-chop cooldown (${cooldownRemaining}min remaining after loss streak)`;
      }
    }
