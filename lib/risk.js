...
export function assessSetupQuality(signal) {
  ...
  return {
    initialRewardRisk: initialRewardRisk === null ? null : Number(initialRewardRisk.toFixed(2)),
...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    if (!setupQuality.rewardOk) {
      const rr = setupQuality.initialRewardRisk === null ? 'unknown' : setupQuality.initialRewardRisk.toFixed(2);
