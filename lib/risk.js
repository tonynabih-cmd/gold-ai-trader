...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    // ── RULE 20: Slippage protection (MOVED to execution.js placeTrade()) ────
    // This check was broken: signal.entryPrice === indicators.lastCandle.close
    // (both set from lastCandle.close in strategy.js), so the difference was
    // always exactly 0 and the guard never fired.
    // The real slippage check now lives in placeTrade() in execution.js:
    // it fetches the live Capital.com bid/ask right before placing the order
    // and aborts if |livePrice - signal.entryPrice| > $1.00.
    // DO NOT re-enable this block — it will never produce a non-zero slippage.
