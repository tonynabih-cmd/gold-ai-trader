...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    // If spread > $0.40, we skip to ensure tightest entry (user-defined threshold).
    // if spread is missing (null/undefined), we also skip for safety.
    if (typeof indicators.spread !== 'number' || isNaN(indicators.spread))
      return 'SKIP: Spread unavailable - skipping for safety';
    
    if (indicators.spread > 0.40)
      return `SKIP: Spread too high ($${indicators.spread.toFixed(2)}) - exceeds $0.40 limit`;
