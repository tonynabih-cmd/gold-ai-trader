...
import { KILL_SWITCH_POLICY, parseActivatedAtMs, repairExpiredKillSwitch } from './kill_switch.js';
...
const ROLLING_PF_WARNING_THRESHOLD = 0.7;
const ROLLING_PF_HARD_THRESHOLD = 0.75;
const RECOVERY_EXPECTANCY_THRESHOLD_R = -0.35;
const RECOVERY_MAX_DURATION_MS = KILL_SWITCH_POLICY.expiryMs;
...
function getExpectancyKillAgeMs(expectancyKill, nowMs = Date.now()) {
  ...
  if (!Number.isFinite(activatedAtMs)) return 0;
  return Math.max(0, nowMs - activatedAtMs);
}
...
export function resetDirectionalLossCircuitOnTrendReset(botState, indicators) {
  ...
  if (expectancyKill.active && expectancyKill.mode === 'HARD_PAUSE') {
    const expired = getExpectancyKillAgeMs(expectancyKill, nowMs) >= RECOVERY_MAX_DURATION_MS;
    ...
    if (trendChanged) {
      ...
      console.log('[RISK] PF10 hard pause reset by 12h cooldown expiry');
      changed = true;
...
export function checkRisk(signal, botState, indicators, options = {}) {
  try {
    ...
    const expectancyKill = ensureExpectancyKillState(botState);
    repairExpiredKillSwitch(botState, now.getTime());
    ...
    const pf5Warning = rollingProfitFactor5 !== null && rollingProfitFactor5 < ROLLING_PF_WARNING_THRESHOLD;
    const expectancyWarning = expectancy5R !== null && expectancy5R < RECOVERY_EXPECTANCY_THRESHOLD_R;
    const currentRecoveryWindowKey = buildOutcomeWindowKey(last5Outcomes);
    const recoveryTrigger = pf5Warning || fullStopsLast3 >= 2 || expectancyWarning;
    const suppressedOldWindow = (
      expectancyKill.suppressedWindowKey &&
      currentRecoveryWindowKey &&
      expectancyKill.suppressedWindowKey === currentRecoveryWindowKey
    );
    const recoveryAgeMs = getExpectancyKillAgeMs(expectancyKill);
    const recoveryCanReset =
      expectancyKill.active &&
      expectancyKill.mode === 'RECOVERY' &&
      (countConsecutiveNonLosses(recentOutcomes) >= 2 || (expectancy5R !== null && expectancy5R >= 0) || recoveryAgeMs >= RECOVERY_MAX_DURATION_MS);

    if (recoveryCanReset) {
      resetExpectancyKillSwitch(expectancyKill, expectancy5R !== null && expectancy5R >= 0 ? 'EXPECTANCY_RECOVERED' : 'RECOVERY_EXIT', currentTrend1h || null);
    } else if (recoveryTrigger && !expectancyKill.active) {
      if (suppressedOldWindow) {
        signal.expectancyKillSuppressedWindow = true;
      }
      if (!suppressedOldWindow) {
        ...
        expectancyKill.windowKey = currentRecoveryWindowKey;
      }
    }
    ...
    if (hardPfFailure) {
      expectancyKill.active = true;
      expectancyKill.mode = 'HARD_PAUSE';
      expectancyKill.activatedAt = expectancyKill.activatedAt || Date.now();
      expectancyKill.activationTrend = currentTrend1h || null;
      expectancyKill.windowKey = buildOutcomeWindowKey(last10Outcomes);
