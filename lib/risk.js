...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    if (grossLossVal > 0) {
      ...
      if (profitFactor < 1.1) {
        botState.botEnabled = false;
        return `DISABLE: Profit Factor (${profitFactor.toFixed(2)}) below minimum threshold (1.1). Bot disabled for performance review.`;
      }
    }
    ...
    // Minimum 10 minutes between trades to prevent overtrading (reduced from 60m).
