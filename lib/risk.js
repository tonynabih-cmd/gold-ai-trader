// risk.js — 18-rule safety gate. Every rule is checked before ANY trade is placed.
...
export function checkRisk(signal, botState, indicators) {
  try {
    ...
    // Capital.com spread for Gold during Golden Hour: typically $0.20–$0.40.
    // If spread > $0.60, conditions are abnormal (thin market or news event).
    // indicators.spread is fetched by market_data.js from the snapshot endpoint.
    if (typeof indicators.spread === 'number' && indicators.spread > 0.60)
    ...
    if (parseFloat(botState.totalDrawdown) >= 20)
      return `DISABLE: Max drawdown reached (${botState.totalDrawdown}%) - bot permanently disabled`;
