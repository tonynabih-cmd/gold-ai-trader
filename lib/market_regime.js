...
export function classifyMarketRegime(indicators = {}) {
  ...
  if (atr < 0.50 || atrRatio < 0.65) return 'DEAD';
  if (atrRatio >= 2.50 || spreadAtr >= 0.45) return 'EXTREME';
  if (atrRatio >= 1.60 || spreadAtr >= 0.30) return 'VOLATILE';
  if (trend1h === 'N/A' || !trend1h || emaSeparationAtr !== null && emaSeparationAtr < 0.12) return 'SIDEWAYS';
