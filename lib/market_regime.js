const ALLOWED_REGIMES = new Set(['NORMAL', 'ACTIVE', 'SOFT_DEAD', 'SOFT_SIDEWAYS', 'SOFT_EXTREME']);
...
export function classifyMarketRegimeDetails(indicators = {}, options = {}) {
  ...
  let regime;
  if (atr < 0.60 || atrRatio !== null && atrRatio < 0.60) {
    regime = 'DEAD';
  } else if (atrRatio !== null && atrRatio < 0.70) {
    regime = 'SOFT_DEAD';
  } else if (atrRatio !== null && atrRatio > 2.60) {
    regime = 'EXTREME';
  } else if (atrRatio !== null && atrRatio > 2.20) {
    regime = 'SOFT_EXTREME';
  } else if (emaSpreadAtr !== null && emaSpreadAtr < 0.14) {
    regime = 'SIDEWAYS';
  } else if (emaSpreadAtr !== null && emaSpreadAtr < 0.18) {
    regime = 'SOFT_SIDEWAYS';
  ...
  return {
    ...
    isAllowedRegime,
    isSoftRegime: String(regime || '').startsWith('SOFT_'),
    riskMultiplier: String(regime || '').startsWith('SOFT_') ? 0.5 : 1.0,
    requiredConfidence: String(regime || '').startsWith('SOFT_') ? 65 : 55,
