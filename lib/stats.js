...
export function computeSessionStats(logs, brokerStats) {
  ...
  if (hasBroker) {
    ...
    // Fallback: extract from ALL closure logs (not just today's session, for accurate historic winrate fallback)
    const allClassified = logs.map(l => ({ log: l, category: classifyLog(l) }));
    const pnls = [];
    for (const { log, category } of allClassified) {
  ...
  if (closedTrades > 0 && (winRate == null || bestTrade == null || worstTrade == null)) {
    throw new Error(`INVARIANT FAILED: Executed trades exist (${closedTrades}), but trade metrics failed calculating.`);
  }
