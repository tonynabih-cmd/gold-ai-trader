...
export function generateSignal(indicators, _) {
  try {
    ...
    if (currEMA20 > currEMA50) {
      ...
      return { signal: null, debug: { ...debugBase, dbgRejectReason: 'EMA20 and EMA50 are flat' } };
    }

    if (action === 'BUY' && indicators.trend1h !== 'UP') {
      return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: `Regime: 1h trend conflict (1h is ${indicators.trend1h})` } };
    }
    ...
    if (!priorExpansion) {
      return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: 'Regime: No prior EMA expansion detected leading into pullback' } };
    }

    const MIN_ATR_FOR_TRADING = 0.50; // Conservative floor for XAUUSD 5m to avoid dead markets

    if (atr < MIN_ATR_FOR_TRADING) {
      return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: `Regime: ATR below minimum threshold (${MIN_ATR_FOR_TRADING.toFixed(2)})` } };
    }
    ...
    if (!previousCandleTouchedZone) {
      return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: `Entry: no EMA20 touch/sweep on prior candle (zone ${touchZone.toFixed(2)})` } };
    }

    const candleSize = lastCandle.high - lastCandle.low;
    if (candleSize === 0) return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: 'Entry: Candle size is 0' } };
    ...
    if (!confirmationValid) {
      return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: 'Entry: weak confirmation or no trend direction close with rejection strength' } };
    }
    ...
    if (isNaN(stopLoss) || isNaN(takeProfit)) return { signal: null, debug: { ...debugBase, dbgAction: action, dbgRejectReason: 'stopLoss or takeProfit is NaN' } };

    const setupConfidence = calculateSetupConfidence({
      action,
      currEMA20,
      currEMA50,
      emaSep1,
    ...
    return {
      signal: {
        ...
        setupConfidence,
        initialRewardRisk: Number(initialRewardRisk.toFixed(2)),
      ...
      debug: {
        ...
        dbgInitialRewardRisk: Number(initialRewardRisk.toFixed(2)),
      },
    };
  } catch (err) {
