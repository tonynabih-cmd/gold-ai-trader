...
export function generateSignal(indicators, _) {
  try {
    ...
    const candleShape = candleShapeDiagnostics(lastCandle);
    const strategyDiagnostics = {
      atrRatio: roundDiagnostic(atrRatio),
      emaSpreadAtr: roundDiagnostic(emaSpreadAtr),
      ...candleShape,
      swingHigh: roundDiagnostic(Math.max(prevCandle.high, lastCandle.high)),
      swingLow: roundDiagnostic(Math.min(prevCandle.low, lastCandle.low)),
      rejectStage: null,
    };
    ...
    const entryDiagnostics = {
      ...
      sweepValid: previousCandleTouchedZone,
    };
