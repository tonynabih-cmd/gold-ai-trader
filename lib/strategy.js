...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    if (entryType === 'crossover' && dbgCrossoverAgeBars === 0) {
      ...
      console.log(`[STRATEGY] Immediate crossover entry allowed: ${action} slope ${slopePercent.toFixed(4)}% >= ${IMMEDIATE_CROSSOVER_SLOPE}%`);
    }
    ...
    if (nearResistance || nearSupport) {
      // Reduce penalty from -2 to -1 if EMA 20/50 separation indicates a strong trend
      const isStrongTrend = dbgEmaSep >= atr * 0.6;
      score -= isStrongTrend ? 1 : 2;
    }
