...
// Three entry types: (1) EMA 20/50 crossover, (2) Pullback/momentum continuation in
// established trend, (3) Breakout on strong momentum spike past EMA20.
// Returns a signal object or null (no trade this cycle).
// riskMultiplier: defaults to 1.0 (full risk); breakout entries use 0.5 (half risk).
//
...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    const CROSSOVER_SLOPE_THRESHOLD = 0.06; 
    const IMMEDIATE_CROSSOVER_SLOPE = 0.25; // 0.25% slope allows skip of 1-candle confirmation
    const TAKE_PROFIT_ATR_MULTIPLIER = 3.0; // Report: 3.0×ATR take-profit (2.5:1 R:R)
    const STOP_LOSS_ATR_MULTIPLIER = 1.2;    // Report: 1.2×ATR stop-loss
    ...
    const PULLBACK_SLOPE_THRESHOLD = isRelaxedMode 
        ? (isEuropeanSession ? 0.05 : 0.07) 
        : (isEuropeanSession ? 0.08 : 0.10);

    // Dynamic ATR-based pullback distance (v1.4)
    // Tighter in low volatility, wider in high volatility.
    const ATR_PULLBACK_MULTIPLIER = isRelaxedMode ? 1.0 : 0.65;
    const pullbackDistanceThreshold = atr * ATR_PULLBACK_MULTIPLIER;

    const momRangeMult = isRelaxedMode ? 0.04 : 0.06;
    ...
    const buyRsiBlock = entryType === 'breakout' ? 75 : (entryType === 'pullback' ? 70 : 65);
    const sellRsiBlock = entryType === 'breakout' ? 25 : (entryType === 'pullback' ? 30 : 35);
    ...
    if (entryType === 'pullback' || entryType === 'momentum' || entryType === 'crossover') {
        if (action === 'BUY') {
          ...
          const minStrongBullish = isRelaxedMode ? 1 : 2;
        ...
        if (action === 'SELL') {
          ...
          const minStrongBearish = isRelaxedMode ? 1 : 2;
