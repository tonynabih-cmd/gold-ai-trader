...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    if (weakSlope) {
      const crossoverOverride = entryType === 'crossover' && dbgEmaSep > atr * 0.5;
      if (!crossoverOverride) {
    ...
    // Trend alignment (already guaranteed by filters above unless override active, so score it)
    let strongTrend = false;
    if (action === 'BUY'  && slopePercent > 0.1)  strongTrend = true;
    if (action === 'SELL' && slopePercent < -0.1) strongTrend = true;

    if (strongTrend) {
      score += 2;
    } else if (entryType === 'crossover' && dbgEmaSep > atr * 0.5) {
      score += 2; // Crossover override compensates for weak slope
    }
