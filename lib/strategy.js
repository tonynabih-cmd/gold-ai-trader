...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    const TAKE_PROFIT_ATR_MULTIPLIER = 1.5;
    ...
    if (!action) {
      ...
      const crossoverLookback = Math.min(6, Math.max((ema20arr?.length || 0) - 1, 0));
    ...
    if (action === 'BUY' && rsi > 70) {
      return { signal: null, debug: { ...signalDebug, dbgRejectReason: `BUY blocked: RSI ${rsi.toFixed(1)} > 70` } };
    }
    if (action === 'SELL' && rsi < 30) {
      return { signal: null, debug: { ...signalDebug, dbgRejectReason: `SELL blocked: RSI ${rsi.toFixed(1)} < 30` } };
    }
    ...
    let weakSlope = false;
    if (action === 'BUY'  && slopePercent < PULLBACK_SLOPE_THRESHOLD)  weakSlope = true;
    if (action === 'SELL' && slopePercent > -PULLBACK_SLOPE_THRESHOLD) weakSlope = true;

    if (weakSlope && entryType !== 'crossover') {
      return { signal: null, debug: { ...signalDebug, dbgRejectReason: `weak EMA slope (${action} requires ${action === 'BUY' ? `>= ${PULLBACK_SLOPE_THRESHOLD}%` : `<= -${PULLBACK_SLOPE_THRESHOLD}%`}, got ${slopePercent.toFixed(4)}%)` } };
    }
    ...
    if (dbg1mCandlesUsed > 0) {
      netMomentum1m = recent1m.reduce((sum, candle) => {
        if (
          ...
          const strongEnough = candleRange >= atr * MOMENTUM_RANGE_MULTIPLIER;
    ...
    if (action === 'BUY') {
      ...
      if (strongBullishCandles < 2) {
        return { signal: null, debug: { ...signalDebug, dbg1mMomentumNet: +netMomentum1m.toFixed(4), dbg1mCandlesUsed, dbgRejectReason: `1m momentum not strong enough (only ${strongBullishCandles}/3 candles reached range >= ${(atr * MOMENTUM_RANGE_MULTIPLIER).toFixed(2)})` } };
      }
    }
    ...
    if (action === 'SELL') {
      ...
      if (strongBearishCandles < 2) {
        return { signal: null, debug: { ...signalDebug, dbg1mMomentumNet: +netMomentum1m.toFixed(4), dbg1mCandlesUsed, dbgRejectReason: `1m momentum not strong enough (only ${strongBearishCandles}/3 candles reached range >= ${(atr * MOMENTUM_RANGE_MULTIPLIER).toFixed(2)})` } };
      }
    }
    ...
    return {
      signal: {
        id:              `${lastCandle.time}_${action}_v1.2`,
        pair:            'GOLD',
        action,
        entryType,
