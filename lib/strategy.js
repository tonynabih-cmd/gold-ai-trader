...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    const CROSSOVER_SLOPE_THRESHOLD = 0.08;
    const TAKE_PROFIT_ATR_MULTIPLIER = 1.8;
    const STOP_LOSS_ATR_MULTIPLIER = 1.35;
    ...
    if (!action && dbgCrossoverAgeBars === 0) {
      ...
      const pullbackDistanceThreshold = emaSeparation < atr * 0.6 ? atr * 0.8 : atr * 1.1;
    ...
    const buyRsiBlock = entryType === 'pullback' ? 67 : 70;
    const sellRsiBlock = entryType === 'pullback' ? 33 : 30;
