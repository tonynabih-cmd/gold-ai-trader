...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    // ── Entry Type 1: EMA 20/50 Crossover (within last 3 bars) ───────────────
    // Looks back up to 3 candles to catch a recent crossover that may have happened
    // between cron invocations. Stops at the first crossover found (most recent).
    if (ema20arr && ema50arr && ema20arr.length >= 4 && ema50arr.length >= 4) {
      ...
      for (let lookback = 1; lookback <= 3; lookback++) {
        ...
        if (typeof prevE20 !== 'number' || typeof prevE50 !== 'number' ||
            typeof currE20 !== 'number' || typeof currE50 !== 'number') continue;
