...
export function generateSignal(indicators, _) {
  try {
    ...
    const emaDist3 = Math.abs(ema20arr[ema20arr.length - 4] - ema50arr[ema50arr.length - 4]);
    const emaDist4 = Math.abs(ema20arr[ema20arr.length - 5] - ema50arr[ema50arr.length - 5]);
    const emaDist5 = Math.abs(ema20arr[ema20arr.length - 6] - ema50arr[ema50arr.length - 6]);
    
    // Check if expansion happened on any recent 3-candle sequence leading into the current state
    const priorExpansion = 
      (emaDist1 > emaDist2 && emaDist2 > emaDist3) ||
      (emaDist2 > emaDist3 && emaDist3 > emaDist4) ||
      (emaDist3 > emaDist4 && emaDist4 > emaDist5);
