...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    if (!action) {
      ...
      // MOMENTUM UPGRADE: If a crossover just happened (last 3 bars), we allow entry 
      // without touching EMA20 if the trend slope is very strong.
      let recentCrossover = false;
      if (ema20arr && ema50arr && ema20arr.length >= 4) {
        for (let i = 1; i <= 3; i++) {
          const idx = ema20arr.length - 1 - i;
          const pE20 = ema20arr[idx];
          const pE50 = ema50arr[idx];
          const cE20 = ema20arr[idx + 1];
          const cE50 = ema50arr[idx + 1];
          if ((pE20 <= pE50 && cE20 > cE50) || (pE20 >= pE50 && cE20 < cE50)) {
      ...
      if (!trendEstablished && !momentumEntryAllowed) {
        ...
        dbgPullbackReason = `pullback: RSI not neutral (RSI ${rsi.toFixed(1)}, need 25–75)`;
      } else {
    ...
    // Minimum momentum thresholds: at least $0.15 net movement
    // This removes near-zero noise like +$0.001 or +$0.05 in choppy markets
    const minMomentumThreshold = 0.15;
    ...
    // ── Build signal object ───────────────────────────────────────────────────
    // Stop loss: 2.0× ATR from entry (increased from 1.5x for safety).
    // Take profit: 3.0× ATR (increased from 2.0x to maintain 1:1.5 R:R).
    const stopLoss = action === 'BUY'
      ? lastCandle.close - (2.0 * atr)
      : lastCandle.close + (2.0 * atr);

    const takeProfit = action === 'BUY'
      ? lastCandle.close + (3.0 * atr)
      : lastCandle.close - (3.0 * atr);
