...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    const pullDistMult = isRelaxedMode ? 0.0020 : 0.0015;
    const pullbackDistanceThreshold = currEMA20 * pullDistMult;
    ...
    if (DEBUG_STRATEGY) {
        ...
        console.log(`[STRATEGY_DEBUG] Slope: ${slopePercent.toFixed(4)}% | RSI: ${rsi.toFixed(1)} | ATR: ${atr.toFixed(2)}`);
        if (action) {
    ...
    if (action === 'BUY') {
      ...
      if (netMomentum1m < minMomentumThreshold) {
        return { signal: null, debug: { ...signalDebug, dbg1mMomentumNet: +netMomentum1m.toFixed(4), dbg1mCandlesUsed, dbgRejectReason: `1m momentum too weak (${netMomentum1m.toFixed(4)} < $${minMomentumThreshold.toFixed(4)})` } };
      }
    ...
    if (action === 'SELL') {
      ...
      if (Math.abs(netMomentum1m) < minMomentumThreshold) {
        return { signal: null, debug: { ...signalDebug, dbg1mMomentumNet: +netMomentum1m.toFixed(4), dbg1mCandlesUsed, dbgRejectReason: `1m momentum too weak (${Math.abs(netMomentum1m).toFixed(4)} < $${minMomentumThreshold.toFixed(4)})` } };
      }
