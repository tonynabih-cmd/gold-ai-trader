...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    let trendMismatch = false;
    if (action === 'BUY'  && trend1h !== 'UP')   trendMismatch = true;
    if (action === 'SELL' && trend1h !== 'DOWN') trendMismatch = true;

    if (trendMismatch) {
      const crossoverOverride = entryType === 'crossover' && dbgEmaSep > atr * 0.5;
      if (!crossoverOverride) {
        return { signal: null, debug: { ...signalDebug, dbgRejectReason: `trend1h mismatch (${action} requires ${action === 'BUY' ? 'UP' : 'DOWN'})` } };
      }
    }
    ...
    if (trend1h === (action === 'BUY' ? 'UP' : 'DOWN')) {
      score += 2;
    } else if (entryType === 'crossover' && dbgEmaSep > atr * 0.5) {
      score += 2; // Crossover override compensates for trend mismatch
    }
