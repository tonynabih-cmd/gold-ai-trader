...
export function generateSignal(indicators, candles1m) {
  try {
    ...
    if (!action && dbgCrossoverAgeBars === 0 && (dbgBuyCrossover || dbgSellCrossover)) {
      ...
      const trendThreshold = 0.35;
      const trendEstablished = emaSeparation > atr * trendThreshold;
      const emaSep = Math.abs(currEMA20 - currEMA50);
      const emaSepPrev = (ema20arr && ema50arr && ema20arr.length >= 4 && ema50arr.length >= 4)
        ? Math.abs(ema20arr[ema20arr.length - 4] - ema50arr[ema50arr.length - 4])
        : emaSep;
      const trend1hAligned = (currEMA20 > currEMA50 && trend1h === 'UP') || (currEMA20 < currEMA50 && trend1h === 'DOWN');

      if (!(emaSep >= 0.50 * atr && (emaSep - emaSepPrev) >= 0.08 * atr)) {
        return { signal: null, debug: { dbgRejectReason: 'EMA expansion filter blocked setup' } };
      }

      if (efficiency12 < 0.30) {
        return { signal: null, debug: { dbgRejectReason: `12-bar efficiency too low (${efficiency12.toFixed(4)} < 0.30)` } };
      }

      if (isRelaxedMode) {
        if (!(emaSep >= 0.60 * atr && efficiency12 >= 0.30 && trend1hAligned)) {
          return { signal: null, debug: { dbgRejectReason: 'relaxed mode blocked: market quality/alignment insufficient' } };
        }
      }

      const distanceToEMA20  = dbgDistToEMA20;
      const touchedEMA20     = distanceToEMA20 < pullbackDistanceThreshold;
      const crossoverLookback = isRelaxedMode ? 18 : 12;
      ...
      if (!trendEstablished && !momentumEntryAllowed) {
        ...
        if (inUptrend) {
          ...
          if (!recentBullishCross && !momentumEntryAllowed && !isRelaxedMode) {
            ...
            if (candleRange === 0) {
              return { signal: null, debug: { dbgRejectReason: 'pullback BUY: zero candle range' } };
            }
            const closePosition = (lastCandle.close - lastCandle.low) / candleRange;
            if (!(lastCandle.low <= currEMA20 + 0.10 * atr && closePosition >= 0.60)) {
              return { signal: null, debug: { dbgRejectReason: 'pullback BUY: weak EMA rejection candle' } };
            }
            action = 'BUY'; 
          ...
          if (!recentBearishCross && !momentumEntryAllowed && !isRelaxedMode) {
            ...
            const candleRange = lastCandle.high - lastCandle.low;
            if (candleRange === 0) {
              return { signal: null, debug: { dbgRejectReason: 'pullback SELL: zero candle range' } };
            }
            const closePosition = (lastCandle.close - lastCandle.low) / candleRange;
            if (!(lastCandle.high >= currEMA20 - 0.10 * atr && closePosition <= 0.40)) {
              return { signal: null, debug: { dbgRejectReason: 'pullback SELL: weak EMA rejection candle' } };
            }
            action = 'SELL'; 
            entryType = momentumEntryAllowed ? 'momentum' : 'pullback';
          }
