...
export function calculateIndicators(candles5m, candles1h) {
  try {
    ...
    // NOTE: Sideways slope filter REMOVED — strategy.js has its own slope checks
    // that are context-aware (relaxed mode, entry type). Pre-filtering here was
    // redundantly blocking cycles that the strategy could have evaluated.
    ...
    // ATR range: 1.2 to 50
    if (currentATR < 1.2 || currentATR > 50) {
      return { skip: true, reason: `SKIP: ATR out of range (1.2-50) (ATR ${currentATR.toFixed(2)})`, ...indicatorData };
    }
