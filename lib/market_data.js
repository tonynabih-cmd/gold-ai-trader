...
export async function getMarketData(session, botState) {
  try {
    ...
    const discarded1h = discardUnclosedCandles(candles1h, HOUR_MS, '1h', timingNowMs);
    const discarded1m = discardUnclosedCandles(candles1m, MINUTE_MS, '1m', timingNowMs);

    console.log(`[DATA] 1h candles fetched: ${candles1h.length} (discarded ${discarded1h} live candle(s))`);
    console.log(`[DATA] 1m candles fetched: ${candles1m.length} (discarded ${discarded1m} live candle(s))`);
    console.log(`[DATA] 5m candles fetched: ${candles5m.length} (discarded ${discarded5m} live candle(s))`);
    ...
    const settlementThreshold = 7; // v1.5: increased to 7s for broker settlement safety
