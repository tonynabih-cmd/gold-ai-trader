...
export const REQUIRED_SETTLEMENT_MS = 7000;
export const CRON_SETTLEMENT_ALIGNMENT_TARGET_MS = 8500;
export const CRON_SETTLEMENT_ALIGNMENT_MAX_DELAY_MS = 10000;
...
export function assessCandleSettlement(candleOpenTimeMs, nowMs, timeframeMs = FIVE_MINUTES_MS) {
  const candleCloseTime = getCandleCloseTime(candleOpenTimeMs, timeframeMs);
  const settlementWaitMs = nowMs - candleCloseTime;
  const settlementPassed = settlementWaitMs >= REQUIRED_SETTLEMENT_MS;

  return {
    candleCloseTime,
    settlementWaitMs,
    requiredSettlementMs: REQUIRED_SETTLEMENT_MS,
    settlementPassed,
  };
}

export function calculateCronSettlementDelayMs(nowMs, timeframeMs = FIVE_MINUTES_MS) {
  const elapsedSinceBoundary = ((nowMs % timeframeMs) + timeframeMs) % timeframeMs;
  if (elapsedSinceBoundary >= CRON_SETTLEMENT_ALIGNMENT_TARGET_MS) return 0;

  const delayMs = CRON_SETTLEMENT_ALIGNMENT_TARGET_MS - elapsedSinceBoundary;
  return delayMs <= CRON_SETTLEMENT_ALIGNMENT_MAX_DELAY_MS ? Math.ceil(delayMs) : 0;
}
...
export async function getMarketData(session, botState) {
  try {
    ...
    // ── Strict Execution Timing (7s delay after close) ──────────────────
    ...
    const {
      ...
      settlementPassed,
    } = assessCandleSettlement(latestCandleTime, settlementNowMs, FIVE_MINUTES_MS);
    const secondsSinceClose = settlementWaitMs / 1000;
    ...
    console.log(
      `[DATA] Settlement Guard: settlementWaitMs=${settlementWaitMs} ` +
      `requiredSettlementMs=${requiredSettlementMs} settlementPassed=${settlementPassed}`
    );
    ...
    console.log(`[DATA] 5m settlement delay this cycle: ${secondsSinceClose.toFixed(3)}s (threshold ${(requiredSettlementMs / 1000).toFixed(0)}s)`);

    if (!settlementPassed) {
      return {
        skip: true,
        reason: `SKIP: Waiting for candle settlement (${secondsSinceClose.toFixed(1)}s since close, need ${(requiredSettlementMs / 1000).toFixed(0)}s)`,
        candles5m,
        candles1h,
        candles1m,
        latestCandleTime,
