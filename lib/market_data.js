...
export async function getMarketData(session, botState) {
  try {
    ...
    // ── Timezone Alignment (Auto-Correction) ──────────────────────────────
    // Capital.com often returns "local" time strings without a timezone suffix.
    // If the server is in UTC and the broker's clock (e.g., UTC+4) returns "22:30",
    // JS parses it as 22:30 UTC — which is 4 hours in the future.
    // We calculate this "drift" and normalize all candles back to true UTC.
    const latestRawTime = candles5m[candles5m.length - 1].time;
    const now           = Date.now();
    // Round difference to nearest hour to find the timezone offset
    const hourMs       = 60 * 60 * 1000;
    const rawDrift     = latestRawTime - now;
    const tzOffsetMs   = Math.round(rawDrift / hourMs) * hourMs;

    if (Math.abs(tzOffsetMs) > 0) {
      console.log(`[DATA] Localizing timestamps: detected ${tzOffsetMs / hourMs}h offset from broker. Aligning to system clock...`);
