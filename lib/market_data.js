...
//
// DESIGN DECISION: Per invocation fetches 1000 5m candles, 60 1h candles,
// 5 1m candles, and a live spread snapshot (no incremental caching).
// Rationale: Vercel is stateless — candles5m cannot persist across invocations without KV.
// Storing 1000 candles in KV is too large and slow. At 96 runs/day × 3 timeframe fetches
// = ~288 API calls/day — well under the Capital.com limit. Simplicity beats fragile caching.
// Settlement delay: 3 seconds after candle close during golden hour (07:00–09:59 UTC),
// 5 seconds otherwise, to allow broker calculations to settle.
...
export async function getMarketData(session, botState) {
  try {
    ...
    if (secondsSinceClose < settlementThreshold) {
      return {
        skip: true,
