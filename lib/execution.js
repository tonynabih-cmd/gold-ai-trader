  ...
  try {
    ...
    if (prevBalance > 0 && realBalance < prevBalance) {
      ...
      botState.dailyLoss = parseFloat(
        (parseFloat(botState.dailyLoss ?? 0) + loss).toFixed(2)
      );
    }
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // Guard against tiny stop distance which would create an enormous position
    if (stopDistance < 0.30) {
      return { success: false, reason: `ERROR: Stop distance too small (${stopDistance.toFixed(3)}) - would create oversized position` };
    }
