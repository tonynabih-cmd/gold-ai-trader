...
const WORST_CASE_MOVE_USD = 20;
const MAX_PORTFOLIO_WORST_CASE_EQUITY_PCT = 0.25;
...
function extractConfirmedSize(confirmData) {
  const direct = parseFloat(confirmData?.size);
  if (!isNaN(direct) && direct > 0) return direct;

  const affected = Array.isArray(confirmData?.affectedDeals) ? confirmData.affectedDeals : [];
  let sum = 0;
  for (const deal of affected) {
    const sz = parseFloat(deal?.size ?? deal?.dealSize);
    if (!isNaN(sz) && sz > 0) sum += sz;
  }
...
function calculatePortfolioWorstCaseRiskAED(openTrades, extraSize = 0) {
  const activeTrades = Array.isArray(openTrades) ? openTrades : [];
  const existingUsd = activeTrades.reduce((sum, t) => {
    const sz = parseFloat(t?.size);
    if (isNaN(sz) || sz <= 0) return sum;
    return sum + (sz * WORST_CASE_MOVE_USD);
  }, 0);
  const extraUsd = extraSize > 0 ? extraSize * WORST_CASE_MOVE_USD : 0;
  return (existingUsd + extraUsd) * USD_AED_PEG;
}
...
export async function verifyExecutionCertainty(session, botState) {
  try {
    ...
    for (const p of brokerPositions) {
      ...
      if (ref) brokerRefs.add(ref);
    }

    const localTrades = Array.isArray(botState.openTrades) ? botState.openTrades : [];
    const localRefs = new Set();
    for (const t of localTrades) {
      if (t?.dealReference) localRefs.add(t.dealReference);
    }

    for (const ref of localRefs) {
      if (!brokerRefs.has(ref)) {
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    const portfolioWorstCaseAED = calculatePortfolioWorstCaseRiskAED(botState.openTrades, positionSize);
    ...
    const confirmedSize = extractConfirmedSize(confirm);
    if (confirmedSize === null || Math.abs(confirmedSize - positionSize) > 0.0001) {
