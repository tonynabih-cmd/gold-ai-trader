...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // 3. Safety: require valid broker stop-distance data before placing any order
    if (minStopDist <= 0) {
      console.warn('[EXEC] Missing broker stop distance data', {
      ...
      return { success: false, reason: 'SKIPPED: Missing broker stop distance data' };
    }

    // 4. Safety: skip trade if broker minimum stop distance is too large vs ATR
    if (minStopDist > signal.atr * MAX_STOP_ATR_RATIO) {
      ...
      return { success: false, reason: 'SKIPPED: stop distance too large vs ATR' };
    }
