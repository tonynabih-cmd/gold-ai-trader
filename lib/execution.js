...
export async function syncBalance(session, botState) {
  try {
    ...
    if (isNaN(realBalance) || realBalance < 0) {
      console.error(`Balance sync: invalid balance value: ${account.balance?.balance}`);
      return botState;
    }
    ...
    // Track daily net loss accurately from start of day balance
    const startOfDay = parseFloat(botState.startOfDayBalance) || prevBalance;
    if (startOfDay > 0 && realBalance < startOfDay) {
      const netLoss = startOfDay - realBalance;
      botState.dailyLoss = parseFloat(netLoss.toFixed(2));
    } else {
      botState.dailyLoss = 0; // Account is in profit for the day (or flat), so daily loss is 0
    }
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // Dynamic sizing capped between 0.01 oz and 1.0 oz
    const MIN_SIZE = 0.01;
    const MAX_SIZE = 1.0;
