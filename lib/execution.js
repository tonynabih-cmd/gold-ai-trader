...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    let mktRes = await fetchWithTimeout(`${baseUrl}/api/v1/markets/GOLD`, {
      headers: {
        'X-CAP-API-KEY': process.env.CAPITAL_API_KEY,
        'CST': cst,
        'X-SECURITY-TOKEN': securityToken,
    ...
    if (!mktRes.ok) {
      // Single retry on transient failure — snapshot is critical for SL/TP and spread gate
      console.warn(`[EXEC] Market snapshot fetch failed (HTTP ${mktRes.status}) — retrying once`);
      await new Promise(r => setTimeout(r, 1500));
      mktRes = await fetchWithTimeout(`${baseUrl}/api/v1/markets/GOLD`, {
        headers: {
          'X-CAP-API-KEY': process.env.CAPITAL_API_KEY,
          'CST': cst,
          'X-SECURITY-TOKEN': securityToken,
    ...
    if (!mktRes.ok) {
      return { success: false, reason: `REJECTED: Market snapshot unavailable (HTTP ${mktRes.status})`, brokerResponse: null };
    }
