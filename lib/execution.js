    ...
    goldTransactions.forEach(t => {
      ...
      if (isToday && (isOpening || isClosure)) {
        ...
        if (dealId && !todayDealIds.has(dealId)) {
          ...
          // If it's a closure, we can't easily see direction in this broker's history,
          // but we can at least count the trade execution itself.
          // Buys and sells stats will be supplemented by livePositions.
        }
      }
    });
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // The signal's SL/TP were computed from the bid-based candle close at signal time.
    // The actual execution price may differ due to spread and market movement.
    // Using stale SL/TP systematically tightens stops on BUY trades.
