...
export async function syncBalance(session, botState) {
  try {
    ...
    const prevBalance = parseFloat(botState.balance);
    ...
    // Update daily loss (only track losses, not gains)
    if (realBalance < prevBalance) {
      const loss = prevBalance - realBalance;
      botState.dailyLoss = parseFloat((parseFloat(botState.dailyLoss || 0) + loss).toFixed(2));
    }
