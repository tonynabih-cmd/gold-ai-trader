...
export async function fetchClosedTradePnl(session, dealReference) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      // Pad 'from' slightly to ensure we don't miss edge cases
      const from = new Date(Date.now() - 25 * 60 * 60 * 1000).toISOString().split('.')[0];
      const to   = new Date().toISOString().split('.')[0];
      
      // Attempt to fetch directly by dealReference if the API supports it as a filter
      // If not, it will just return the standard list for the time window.
      const url = `${baseUrl}/api/v1/history/transactions?from=${from}&to=${to}&type=TRADE&dealId=${dealReference}`;
      ...
      if (attempt < MAX_ATTEMPTS) {
        console.log(`fetchClosedTradePnl: P&L not found for ${dealReference}. Retrying in ${RETRY_DELAY/1000}s... (Attempt ${attempt}/${MAX_ATTEMPTS})`);
