...
function extractStrictFilledSize(confirmData, expectedSize = null) {
  ...
  // Extract actual dealId from the broker's resulting position
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // Use the actual position dealId from affectedDeals if available, otherwise fallback to the order dealReference
    const finalDealReference = fillValidation.actualDealId || dealReference;
    ...
    console.log(`[EXEC] ✅ TRADE OPENED: ${signal.action} ${positionSize}oz GOLD @ ${executionPrice.toFixed(2)} | SL=${signal.stopLoss.toFixed(2)} TP=${signal.takeProfit.toFixed(2)} | ref=${finalDealReference} | risk=$${actualRiskDollars.toFixed(2)}`);
    const saved = await saveStateCritical(botState, `trade_opened:${finalDealReference}`);
    if (!saved) {
