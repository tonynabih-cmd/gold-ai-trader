...
// Capital.com GOLD CFD constants
...
const MAX_RISK_PCT      = 0.02;   // Max risk per trade as % of balance (2%) — scales at any account size
...
export async function syncBalance(session, botState) {
  try {
    ...
    console.log(
      `Balance synced: $${realBalance.toFixed(2)} | ` +
      `Available margin: $${availableMargin.toFixed(2)} | ` +
      `Peak: $${botState.peakBalance.toFixed(2)} | ` +
 ...
 */
export async function fetchClosedTradePnl(session, dealReference) {
  try {
    const { baseUrl, cst, securityToken } = session;
    // Look back 24 hours to find the closing transaction
    const from = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString().split('.')[0];
    const to   = new Date().toISOString().split('.')[0];
    
    // Capital.com history/transactions?type=TRADE
    const res = await fetchWithTimeout(
      `${baseUrl}/api/v1/history/transactions?from=${from}&to=${to}&type=TRADE`,
      {
        headers: {
          'X-CAP-API-KEY':    process.env.CAPITAL_API_KEY,
          'CST':              cst,
    ...
    // Find the transaction that matches our dealReference AND is a closing action (profitAndLoss present)
    const tx = transactions.find(t => t.dealReference === dealReference && t.profitAndLoss != null);
    ...
    console.warn(`fetchClosedTradePnl error: ${err.message}`);
    return null;
  }
}
...
// Leverage-aware position sizing for Capital.com GOLD CFD.
//
// KEY INSIGHT: On a CFD, P&L is determined by:
//   profit/loss = size (oz) × price_change ($/oz)
//   This has NOTHING to do with leverage — leverage only affects margin.
//
// So the correct formula IS:
//   size = riskDollars / stopDistance
//
// The LEVERAGE matters for margin check only:
//   marginRequired = size × currentPrice × marginRate (5%)
//
// We apply THREE safety caps:
//   1. MAX_RISK_PCT cap: size ≤ (balance × MAX_RISK_PCT) / stopDistance
//   2. MAX_SIZE cap: 1.0 oz absolute ceiling
//   3. Margin check: marginRequired × MARGIN_BUFFER ≤ availableMargin
//
export function calculatePositionSize(balance, stopDistance, currentPrice, availableMargin) {
  // ── Validate inputs ──────────────────────────────────────────────────────
  if (isNaN(balance) || balance <= 0)          return { size: 0, error: 'Invalid balance' };
  if (isNaN(stopDistance) || stopDistance <= 0) return { size: 0, error: 'Invalid stop distance' };
  if (isNaN(currentPrice) || currentPrice <= 0) return { size: 0, error: 'Invalid current price' };

  // ── Minimum stop distance guard ──────────────────────────────────────────
  if (stopDistance < MIN_STOP_DISTANCE) {
    return {
      size:  0,
      error: `Stop distance $${stopDistance.toFixed(3)} is below minimum $${MIN_STOP_DISTANCE} — would create oversized position`,
    };
  }

  // ── Risk-based sizing (1% of balance, hard-capped at 2% of balance) ────────
  const maxRiskDollars    = balance * MAX_RISK_PCT;           // 2% of live balance — scales at any account size
  const risk1Pct          = balance * 0.01;                  // 1% of account
  const riskAmount        = Math.min(risk1Pct, maxRiskDollars); // target 1%, ceiling 2%
  const sizeFromRisk      = riskAmount / stopDistance;
  ...
  // ── Actual risk at this size (may differ from riskAmount if floored) ─────
  const actualRiskDollars = parseFloat((positionSize * stopDistance).toFixed(2));

  // ── Hard cap enforcement ─────────────────────────────────────────────────
  // If even the minimum size risks more than maxRiskDollars, skip the trade.
  if (actualRiskDollars > maxRiskDollars) {
    return {
      size:  0,
      error: `Even minimum size (${MIN_SIZE}oz) risks $${actualRiskDollars.toFixed(2)}, exceeding ${(MAX_RISK_PCT * 100).toFixed(0)}% balance cap ($${maxRiskDollars.toFixed(2)})`,
    };
  }

  // ── Margin calculation ───────────────────────────────────────────────────
  const notionalValue    = positionSize * currentPrice;   // Total USD exposure
  const marginRequired   = notionalValue * GOLD_MARGIN_RATE;
  const marginWithBuffer = marginRequired * MARGIN_BUFFER;

  // ── Margin availability check ────────────────────────────────────────────
  if (availableMargin < marginWithBuffer) {
    return {
      size:  0,
      error: `Insufficient margin: need $${marginWithBuffer.toFixed(2)} (with ${MARGIN_BUFFER}× buffer), have $${availableMargin.toFixed(2)}`,
    };
  }
