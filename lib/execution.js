...
export async function verifyExecutionCertainty(session, botState) {
  try {
    ...
    for (const [dealId, localPos] of localByDealId) {
      ...
      if (!brokerPos) {
        // Find the trade in botState to check missingCount (API delay tolerance)
        const trade = localTrades.find(t => String(t.dealId) === String(dealId));
        const missingCount = trade?.missingCount || 0;

        if (missingCount < 3) {
          console.warn(`[CERTAINTY] Local dealId ${dealId} NOT ON BROKER, but missingCount is ${missingCount}. Tolerating sync delay.`);
          continue;
        }

        // This is the core mismatch. We return a specific reason so the caller
        // can handle retry logic (e.g. missingCount) rather than an instant halt.
        console.warn(`[CERTAINTY] Local dealId ${dealId} NOT ON BROKER after ${missingCount} cycles. Matches: ${brokerByDealId.size}`);
 ...
 */
export async function fetchClosedTradePnl(session, targetId, openedAt) {
  try {
    const { baseUrl, cst, securityToken } = session;

    let from;
    if (openedAt && !isNaN(openedAt)) {
      // Look back from 1 hour before it was opened to be safe
    ...
    // Search for a transaction that matches our dealId
    const tx = transactions.find(t => {
      // Primary match on dealId
      const idMatch = (String(t.dealId) === String(targetId)) || 
                      (String(t.dealReference) === String(targetId)) || 
                      (String(t.reference) === String(targetId));
      
      if (!idMatch) return false;
      
      // Must be a closing transaction or have P&L
    ...
    console.error(`[EXEC] fetchClosedTradePnl error for ${targetId}:`, err.message);
    return null;
  }
}
