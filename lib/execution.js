...
function extractStrictFilledSize(confirmData, expectedSize = null) {
  ...
  if (affected.length === 0) {
    if (Number.isFinite(rootSize) && rootSize > 0) {
      return { ok: true, reason: 'ROOT_SIZE_FOUND', filledSize: rootSize };
    }
  ...
  if (Number.isFinite(rootSize) && rootSize > 0) {
    return { ok: true, reason: 'FALLBACK_ROOT_SIZE', filledSize: rootSize };
  }
  ...
  if (affected.length === 1 && expectedSize > 0) {
    ...
    return { ok: true, reason: 'ADOPTED_EXPECTED_SIZE', filledSize: expectedSize };
  }
