...
const EXECUTION_STOP_LOSS_ATR_MULTIPLIER = 1.5;   // v1.5: was 1.2 — +25% buffer absorbs spread + slippage so real loss ≈ 1R
const EXECUTION_TAKE_PROFIT_ATR_MULTIPLIER = 2.5;  // v1.5: was 3.0 — tighter TP that actually gets hit (1.67:1 R:R)
...
export function calculatePositionSize(balanceAED, stopDistanceUSD, currentPriceUSD, availableMarginAED, riskMultiplier = 1.0) {
  ...
  const riskAmountUSD     = balanceUSD * activeRiskPct;             // default 0.75% of account, adjusted by multiplier
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    if (slippage > 1.00) {
      return { success: false, reason: `REJECTED: Slippage too high ($${slippage.toFixed(2)} > $1.00)` };
    }
