...
const EXECUTION_STOP_LOSS_ATR_MULTIPLIER = 2.0;   // v1.5: was 1.5 — increased for broker SL min-distance compatibility
const EXECUTION_TAKE_PROFIT_ATR_MULTIPLIER = 2.5;  // v1.5: was 3.0 — tighter TP that actually gets hit (1.25:1 R:R with 2R stop)
const MAX_SLIPPAGE_BASE = 3.00;                   // v1.5: was 1.00 — increased for current gold volatility
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    if (signal.timestamp && (Date.now() - signal.timestamp) > MAX_SIGNAL_AGE_MS) {
      return { success: false, reason: `REJECTED: Signal is stale (${Math.round((Date.now() - signal.timestamp) / 1000)}s old)` };
    }
    ...
    const maxSpread = parseFloat(process.env.MAX_SPREAD) || 0.5;
    if (spread > maxSpread) {
    ...
    if (slippage > slippageLimit) {
      return { success: false, reason: `REJECTED: Slippage too high ($${slippage.toFixed(2)} > allowed limit $${slippageLimit.toFixed(2)})` };
    }
    ...
    // 2. Enforce Broker Minimum Distance for Guaranteed Stops
    // Capital.com 'snapshot' carries minControlledRiskStopDistance (guaranteed)
    const minStopDist = parseFloat(snapshot.minControlledRiskStopDistance?.value || snapshot.minNormalStopDistance?.value || 0);
    const actualDist = Math.abs(executionPrice - tentativeSL);
    
    if (minStopDist > 0 && actualDist < minStopDist) {
      console.log(`[EXEC] Stopping tightening: ATR stop dist ${actualDist.toFixed(2)} is thinner than broker minimum ${minStopDist.toFixed(2)}. Adjusting...`);
      tentativeSL = signal.action === 'BUY'
        ? executionPrice - (minStopDist + 0.10) // Add tiny buffer to ensure acceptance
        : executionPrice + (minStopDist + 0.10);
    }
    ...
    const orderBody = {
      ...
      size:          positionSize,
      guaranteedStop: true,   // v1.5: guaranteed stop prevents gap slippage (higher fees but SL integrity)
