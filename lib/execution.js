...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // ── Live slippage check & capture actual market price ───────────────────────
    let executionPrice = signal.entryPrice; // Default to signal price
    try {
      ...
      if (mktRes.ok) {
        ...
        if (!isNaN(liveBid) && !isNaN(liveAsk)) {
          ...
          if (slippage > 1.00) {
            return { success: false, reason: `REJECTED: Slippage too high ($${slippage.toFixed(2)}) — live price $${livePrice.toFixed(2)} vs signal $${signal.entryPrice.toFixed(2)}. Trade cancelled.` };
          }
          // Use actual market price for all calculations
          executionPrice = livePrice;
        } else {
          console.warn('[EXEC] Slippage check: could not parse live bid/ask — using signal price for sizing');
        }
      } else {
        console.warn(`[EXEC] Slippage check: markets endpoint returned ${mktRes.status} — using signal price for sizing`);
      }
    } catch (slippageErr) {
      console.warn(`[EXEC] Slippage check error: ${slippageErr.message} — using signal price for sizing`);
    }
