...
function extractStrictFilledSize(confirmData, expectedSize = null) {
  ...
  // Extract actual dealId from the broker's resulting position (this is the true position ID)
  const actualDealId = affected.length > 0 ? (affected[0]?.dealId || affected[0]?.dealReference) : null;
  ...
  // Capital.com v1 confirms often omit size if it matches the request.
  if (affected.length === 1 && expectedSize > 0) {
    const dealId = affected[0]?.dealId || 'unknown';
...
export async function verifyExecutionCertainty(session, botState) {
  try {
    ...
    for (const p of brokerPositions) {
      ...
      const ref = normalizeId(p.position?.dealReference);
      
      const primaryId = dealId || ref;
      if (!primaryId) {
        return { ok: false, reason: 'EXECUTION_STATE_UNCERTAIN:BROKER_POSITION_MISSING_ID' };
      }
      ...
      if (brokerSize === null || brokerDirection === null) {
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:INVALID_BROKER_POSITION:${primaryId}` };
      }

      brokerByDealId.set(primaryId, {
        size: brokerSize,
    ...
    for (const t of localTrades) {
      ...
      const ref = normalizeId(t?.dealReference);
      
      const primaryId = dealId || ref;
      if (!primaryId) {
        return { ok: false, reason: 'EXECUTION_STATE_UNCERTAIN:LOCAL_TRADE_MISSING_ID' };
      }
      ...
      if (localSize === null || localDirection === null) {
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:INVALID_LOCAL_POSITION:${primaryId}` };
      }

      localByDealId.set(primaryId, {
        size: localSize,
    ...
    for (const [id, localPos] of localByDealId) {
      const brokerPos = brokerByDealId.get(id);
      if (!brokerPos) {
        // Mismatch detected. We don't halt immediately to allow for API sync delays.
        // The reconciliation loop in cron.js handles the missingCount/retry logic.
        // verifyExecutionCertainty is the ultimate gate.
        console.warn(`[CERTAINTY] Local trade ${id} not found on broker during certainty check. Matches so far: ${brokerByDealId.size}`);
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:LOCAL_NOT_ON_BROKER:${id}` };
      }

      if (!sizesMatchExactly(localPos.size, brokerPos.size)) {
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:SIZE_MISMATCH:${id} (local=${localPos.size}, broker=${brokerPos.size})` };
      }

      if (localPos.direction !== brokerPos.direction) {
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:DIRECTION_MISMATCH:${id} (local=${localPos.direction}, broker=${brokerPos.direction})` };
      }
    }

    for (const id of brokerByDealId.keys()) {
      if (!localByDealId.has(id)) {
        console.warn(`[CERTAINTY] Broker position ${id} not found locally during certainty check.`);
        return { ok: false, reason: `EXECUTION_STATE_UNCERTAIN:BROKER_NOT_LOCAL:${id}` };
      }
    }
...
export async function fetchClosedTradePnl(session, dealReference, openedAt) {
  try {
    ...
    const tx = transactions.find(t => {
      const idMatch = (t.dealId && (t.dealId === dealReference)) || 
                      (t.dealReference && (t.dealReference === dealReference)) || 
                      (t.reference && (t.reference === dealReference));
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    if (!actualDealId) {
      console.warn(`[EXEC] Could not extract actualDealId for ${dealReference}; falling back to reference`);
    }
    const finalDealId = actualDealId || dealReference;
    ...
    console.log(`[EXEC] ✅ TRADE OPENED: ${signal.action} ${positionSize}oz GOLD @ ${executionPrice.toFixed(2)} | SL=${signal.stopLoss.toFixed(2)} TP=${signal.takeProfit.toFixed(2)} | dealId=${finalDealId} | ref=${dealReference} | risk=$${actualRiskDollars.toFixed(2)}`);
    ...
    return {
      success:          true,
      dealReference:    finalDealReference,
      size:             positionSize,
      entry:            executionPrice,
      actualRiskDollars,
      notionalValue,
