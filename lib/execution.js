...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // ── Live slippage check ───────────────────────────────────────────────────
    // Fetch the real-time bid/ask from Capital.com right before placing the order.
    // signal.entryPrice = lastCandle.close (stale by up to 5 minutes).
    // If price has moved > $1.00 since the signal was generated, abort.
    try {
      ...
      if (mktRes.ok) {
        ...
        if (!isNaN(liveBid) && !isNaN(liveAsk)) {
          ...
          console.log(`Slippage check: signal=${signal.entryPrice.toFixed(2)}, live=${livePrice.toFixed(2)}, slippage=$${slippage.toFixed(2)}`);
          if (slippage > 1.00) {
          ...
          console.warn('Slippage check: could not parse live bid/ask — proceeding without slippage guard');
        }
      } else {
        console.warn(`Slippage check: markets endpoint returned ${mktRes.status} — proceeding without slippage guard`);
      }
    } catch (slippageErr) {
      console.warn(`Slippage check error: ${slippageErr.message} — proceeding without slippage guard`);
    }
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        console.log(`placeTrade: Attempt ${attempt}/${MAX_RETRIES} for ${signal.action} ${positionSize}oz GOLD`);
        ...
        try { result = await res.json(); } catch (e) {
          if (attempt < MAX_RETRIES) {
            console.warn(`placeTrade: invalid JSON response on attempt ${attempt}. Retrying...`);
        ...
        if (!res.ok || result.errorCode) {
          // If it's a specific error like "not enough money", don't retry
          ...
          console.warn(`placeTrade: error ${result.errorCode} on attempt ${attempt}. Retrying in 2s...`);
          await new Promise(r => setTimeout(r, 2000));
          continue;
        }
        ...
        console.warn(`placeTrade: network error on attempt ${attempt}: ${err.message}. Retrying...`);
        await new Promise(r => setTimeout(r, 2000));
      }
    }
