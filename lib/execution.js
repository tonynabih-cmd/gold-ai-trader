...
import { saveStateCritical, saveBadFillResponse } from './state.js';
...
// Attempts to extract the filled position size from a deal confirmation response.
// Uses optional chaining and nullish coalescing throughout — never assumes any
// nested field exists.  Returns the best-effort size plus any diagnostic warnings.
// Never throws.
export function extractBestEffortFilledSize(confirmData) {
  const warnings = [];

  // ── Preferred path: sum sizes from affectedDeals array ───────────────────
  const affected = Array.isArray(confirmData?.affectedDeals) ? confirmData.affectedDeals : [];
  if (affected.length === 0) {
    warnings.push('affectedDeals missing or empty');
  } else {
    let sum = 0;
    for (const deal of affected) {
      const sz = Number(deal?.size ?? deal?.dealSize ?? deal?.filledSize);
      if (!Number.isFinite(sz) || sz <= 0) {
        warnings.push(`Skipping invalid size in affectedDeal (size=${JSON.stringify(deal?.size)}, dealSize=${JSON.stringify(deal?.dealSize)}, filledSize=${JSON.stringify(deal?.filledSize)})`);
        continue;
      }
      sum += sz;
    }
    if (sum > 0) {
      return { filledSize: sum, source: 'affectedDeals', warnings };
    }
    warnings.push('affectedDeals present but no valid sizes could be extracted');
  }

  // ── Fallback: top-level size fields on the confirm object ────────────────
  const topSz = Number(confirmData?.size ?? confirmData?.dealSize ?? confirmData?.filledSize);
  if (Number.isFinite(topSz) && topSz > 0) {
    warnings.push('Used top-level size field as fallback — affectedDeals was unavailable');
    return { filledSize: topSz, source: 'topLevel', warnings };
  }

  warnings.push('No valid fill size found in affectedDeals or top-level fields');
  return { filledSize: null, source: null, warnings };
}
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    // ── Fill parsing (resilient) ───────────────────────────────────────────
    // Wrapped in try/catch so any unexpected parse error is caught gracefully.
    // Never sets botEnabled=false or criticalFailure=true for a bad fill response.
    let filledSize;
    try {
      const fillResult = extractBestEffortFilledSize(confirm);

      if (fillResult.warnings.length > 0) {
        for (const w of fillResult.warnings) {
          console.warn(`[EXEC] Fill warning (${dealReference}): ${w}`);
        }
      }

      if (fillResult.filledSize === null) {
        // Cannot determine fill size — alert user and return early without halting
        console.error(`[EXEC] ⚠️ Fill parse failed for ${dealReference} — unable to determine filled size`);
        await saveBadFillResponse({ dealReference, confirm, timestamp: Date.now() }).catch(() => {});
        await sendAlert(`⚠️ Fill parse failed — trade may be open on Capital.com, check manually`).catch(() => {});
        botState.pendingOrder = null;
        await saveStateCritical(botState, `fill_parse_failed:${dealReference}`);
        return { success: false, reason: 'WARN: Fill parse failed — trade may be open on Capital.com, check manually' };
      }

      filledSize = fillResult.filledSize;

      if (!sizesMatchExactly(filledSize, positionSize)) {
        console.warn(`[EXEC] ⚠️ Fill size mismatch (${dealReference}): filled=${filledSize}, requested=${positionSize} — recording actual filled size`);
      }
    } catch (fillParseErr) {
      console.error(`[EXEC] ⚠️ Fill parse exception for ${dealReference}:`, fillParseErr.message);
      await saveBadFillResponse({ dealReference, confirm, parseError: fillParseErr.message, timestamp: Date.now() }).catch(() => {});
      await sendAlert(`⚠️ Fill parse failed — trade may be open on Capital.com, check manually`).catch(() => {});
      botState.pendingOrder = null;
      await saveStateCritical(botState, `fill_parse_exception:${dealReference}`);
      return { success: false, reason: 'WARN: Fill parse exception — trade may be open on Capital.com, check manually' };
    }
    ...
    console.log(`[EXEC] ✅ TRADE OPENED: ${signal.action} ${filledSize}oz GOLD @ ${executionPrice.toFixed(2)} | SL=${signal.stopLoss.toFixed(2)} TP=${signal.takeProfit.toFixed(2)} | ref=${dealReference} | risk=$${actualRiskDollars.toFixed(2)}`);
    ...
    const postOpenCertainty = await verifyExecutionCertainty(session, botState);
    if (!postOpenCertainty.ok) {
      botState.botEnabled = false;
      botState.stateIntegrityOk = false;
      botState.criticalFailure = true;
      botState.criticalFailureReason = postOpenCertainty.reason;
      await saveStateCritical(botState, `post_trade_open_certainty_failed:${dealReference}:${postOpenCertainty.reason}`);
      return { success: false, reason: `CRITICAL_FAILURE: ${postOpenCertainty.reason}` };
    }
