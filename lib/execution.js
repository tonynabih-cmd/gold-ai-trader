...
export async function fetchBrokerTradeStats(session) {
  try {
    ...
    goldTransactions.forEach(t => {
      ...
      if (isNaN(pnlValue) && t.note?.includes('closed')) {
        pnlValue = parseFloat(t.size);
      }
    ...
    const sessionWorst      = sessionTotalTrades > 0 ? Math.min(...pnlsToday) : 0; // Fixed sessionTotalTrades error below
