 ...
 * This is the SINGLE SOURCE OF TRUTH for all trade metrics:
 *   - Win rate, best/worst trade, total P&L
 *   - Daily trade count (for limit enforcement)
 * The old approach of relying on closure log P&L was unreliable because
 * fetchClosedTradePnl() frequently fails. This function gets the data
 * directly from Capital.com's transaction history API.
 */
export async function fetchBrokerTradeStats(session) {
  try {
    const { baseUrl, cst, securityToken } = session;
    
    // Look back 30 days to capture ALL recent trades (not just 48h)
    // Capital.com's transaction history API supports wide date ranges.
    ...
    if (!res.ok) {
      console.warn(`fetchBrokerTradeStats: API error ${res.status}`);
      return null;
    }
    ...
    // Filter for Gold (XAU) trades only
    const goldTrades = transactions.filter(t => t.instrumentName?.includes('GOLD'));
    
    // Calculate Today's trades (UAE Time)
    const uaeDate = new Date(new Date().getTime() + (4 * 60 * 60 * 1000));
    const todayStr = uaeDate.toISOString().slice(0, 10);
    
    const todayTrades = goldTrades.filter(t => {
      const tDate = new Date(new Date(t.date).getTime() + (4 * 60 * 60 * 1000)).toISOString().slice(0, 10);
      return tDate === todayStr;
    });

    // ── Per-trade P&L metrics ─────────────────────────────────────────────
    // Extract P&L from every closed trade to compute win rate, best/worst.
    const pnls = [];
    goldTrades.forEach(t => {
      const pnl = parseFloat(t.profitAndLoss);
      if (!isNaN(pnl)) {
        pnls.push(pnl);
      }
    });
    ...
    const winRate    = pnls.length > 0 ? parseFloat(((wins / pnls.length) * 100).toFixed(2)) : null;
    const bestTrade  = pnls.length > 0 ? Math.max(...pnls) : null;
    const worstTrade = pnls.length > 0 ? Math.min(...pnls) : null;

    console.log(
      `fetchBrokerTradeStats: ${goldTrades.length} trades | ` +
      `Wins: ${wins} | Losses: ${losses} | Win rate: ${winRate}% | ` +
      `P&L: ${totalPnl.toFixed(2)} | Best: ${bestTrade} | Worst: ${worstTrade}`
    );
    ...
    return {
      ...
      bestTrade,
      worstTrade,
      pnls,            // raw per-trade P&L array for any downstream analysis
...
export function calculatePositionSize(balanceAED, stopDistanceUSD, currentPriceUSD, availableMarginAED) {
  ...
  // ── Risk-based sizing (0.5% of balance, hard-capped at 1% of balance) ──────
  const maxRiskUSD        = balanceUSD * 0.01;              // 1% of live balance
  const targetRiskUSD     = balanceUSD * 0.005;             // 0.5% of account
