 ...
 * Used to synchronize the dashboard and strictly enforce daily trade limits.
 */
export async function fetchBrokerTradeStats(session) {
  try {
    const { baseUrl, cst, securityToken } = session;
    
    // Look back 48 hours for a complete picture (captures yesterday and today)
    ...
    // Calculate total P&L from transaction history
    let totalPnl = 0;
    goldTrades.forEach(t => {
      totalPnl += parseFloat(t.profitAndLoss || 0);
    });
