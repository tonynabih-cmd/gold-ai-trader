...
const RISK_PCT          = 0.0075; // Target risk per trade as % of balance (0.75%)
...
const EXECUTION_STOP_LOSS_ATR_MULTIPLIER = 1.5;
const EXECUTION_TAKE_PROFIT_ATR_MULTIPLIER = 2.25;
...
export async function placeTrade(session, signal, botState) {
  try {
    ...
    console.log(`[EXEC] ✅ TRADE OPENED (${signal.entryType}): ${signal.action} ${positionSize}oz GOLD @ ${executionPrice.toFixed(2)} | SL=${adjustedSL.toFixed(2)} TP=${adjustedTP.toFixed(2)} | dealId=${finalDealId} | ref=${dealReference} | risk=$${actualRiskDollars.toFixed(2)}`);
