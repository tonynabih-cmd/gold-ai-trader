...
export const CYCLE_LOG_PRIMARY_KEY = 'trade_logs_list';
export const CYCLE_LOG_FALLBACK_KEYS = ['cycle_logs'];
...
function firstBoolOrNull(...values) {
  for (const value of values) {
...
function deriveRegimeThresholdTelemetry(regimeDetails, indicators = {}) {
  ...
  return {
    ...
    atrDeadDistance: atrRatioValue !== null
      ? roundTelemetry(atrRatioValue - 0.60)
      : atr !== null
        ? roundTelemetry(atr - 0.60)
        : null,
    atrExtremeDistance: atrRatioValue !== null ? roundTelemetry(2.60 - atrRatioValue) : null,
    emaSpreadAtrValue: roundTelemetry(emaSpreadAtrValue),
    sidewaysDistance: emaSpreadAtrValue !== null ? roundTelemetry(emaSpreadAtrValue - 0.14) : null,
  };
}
...
export const V2_DIAGNOSTIC_FIELDS = [
  ...
  'emaExpansionMissing',
  'emaExpansionPenalty',
  'emaExpansionHandledAs',
  'penaltyReason',
  'rejectStage',
  'strategyVersion',
  'settlementWaitMs',
  'requiredSettlementMs',
  'settlementPassed',
  'highQualityOverride',
  'highQualityOverrideReason',
  'overrideScoreThreshold',
  'overrideRrThreshold',
  'sameDirectionOpenTrade',
  'existingTradeReached1R',
  'clusteringDecision',
  'clusteringReason',
  'emaTouchToleranceAtr',
  'emaTouchDistanceAtr',
  'emaTouchPassedByTolerance',
  'wouldPassSoftRegime',
  'softRegimeRiskMultiplier',
  'requiredConfidenceUsed',
  'confirmationDisplacementAtr',
  'oppositeWickPct',
  'entryExtensionAtr',
  'recoveryModeActive',
  'pf5Telemetry',
...
export const LOGGER_EXPORT_FIELDS = [
  'blockedSetupMfe1hR',
  'blockedSetupMae1hR',
  'blockedSetupMfe3hR',
  'blockedSetupMae3hR',
...
export function buildV2Diagnostics(data = {}, marketRegime = null, now = new Date()) {
  ...
  return {
    ...
    trendConflict: boolOrNull(valueOrNull(
      data.trendConflict,
      signalDebug.trendConflict,
      signalDebug.dbgTrendConflict,
      signal.trendConflict,
      signal.setupConfidence?.trendConflict
    )),
    trendConflictPenalty: finiteOrNull(valueOrNull(
      data.trendConflictPenalty,
      signalDebug.trendConflictPenalty,
      signalDebug.dbgTrendConflictPenalty,
      signal.trendConflictPenalty,
      signal.setupConfidence?.trendConflictPenalty
    )),
    emaExpansionMissing: boolOrNull(valueOrNull(
      data.emaExpansionMissing,
      signalDebug.emaExpansionMissing,
      signalDebug.dbgEmaExpansionMissing,
      signal.emaExpansionMissing,
      signal.setupConfidence?.emaExpansionMissing
    )),
    emaExpansionPenalty: finiteOrNull(valueOrNull(
      data.emaExpansionPenalty,
      signalDebug.emaExpansionPenalty,
      signalDebug.dbgEmaExpansionPenalty,
      signal.emaExpansionPenalty,
      signal.setupConfidence?.emaExpansionPenalty
    )),
    emaExpansionHandledAs: valueOrNull(
      data.emaExpansionHandledAs,
      signalDebug.emaExpansionHandledAs,
      signalDebug.dbgEmaExpansionHandledAs,
      signal.emaExpansionHandledAs,
      signal.setupConfidence?.emaExpansionHandledAs
    ),
    penaltyReason: valueOrNull(
      data.penaltyReason,
      signalDebug.penaltyReason,
      signalDebug.dbgPenaltyReason,
      signal.penaltyReason,
      signal.setupConfidence?.penaltyReason
    ),
    ...
    settlementWaitMs: finiteOrNull(valueOrNull(data.settlementWaitMs, signalDebug.settlementWaitMs)),
    requiredSettlementMs: finiteOrNull(valueOrNull(data.requiredSettlementMs, signalDebug.requiredSettlementMs)),
    settlementPassed: boolOrNull(valueOrNull(data.settlementPassed, signalDebug.settlementPassed)),
    highQualityOverride: boolOrNull(valueOrNull(
      data.highQualityOverride,
      signalDebug.highQualityOverride,
      signal.highQualityOverride,
      signal.setupQuality?.highQualityOverride
    )),
    highQualityOverrideReason: valueOrNull(
      data.highQualityOverrideReason,
      signalDebug.highQualityOverrideReason,
      signal.highQualityOverrideReason,
      signal.setupQuality?.highQualityOverrideReason
    ),
    overrideScoreThreshold: finiteOrNull(valueOrNull(
      data.overrideScoreThreshold,
      signalDebug.overrideScoreThreshold,
      signal.overrideScoreThreshold,
      signal.setupQuality?.overrideScoreThreshold
    )),
    overrideRrThreshold: finiteOrNull(valueOrNull(
      data.overrideRrThreshold,
      signalDebug.overrideRrThreshold,
      signal.overrideRrThreshold,
      signal.setupQuality?.overrideRrThreshold
    )),
    sameDirectionOpenTrade: boolOrNull(valueOrNull(
      data.sameDirectionOpenTrade,
      signalDebug.sameDirectionOpenTrade,
      signal.sameDirectionOpenTrade
    )),
    existingTradeReached1R: boolOrNull(valueOrNull(
      data.existingTradeReached1R,
      signalDebug.existingTradeReached1R,
      signal.existingTradeReached1R
    )),
    clusteringDecision: valueOrNull(
      data.clusteringDecision,
      signalDebug.clusteringDecision,
      signal.clusteringDecision
    ),
    clusteringReason: valueOrNull(
      data.clusteringReason,
      signalDebug.clusteringReason,
      signal.clusteringReason
    ),
    emaTouchToleranceAtr: finiteOrNull(valueOrNull(
      data.emaTouchToleranceAtr,
      signalDebug.emaTouchToleranceAtr
    )),
    emaTouchDistanceAtr: finiteOrNull(valueOrNull(
      data.emaTouchDistanceAtr,
      signalDebug.emaTouchDistanceAtr
    )),
    emaTouchPassedByTolerance: boolOrNull(valueOrNull(
      data.emaTouchPassedByTolerance,
      signalDebug.emaTouchPassedByTolerance
    )),
    wouldPassSoftRegime: boolOrNull(valueOrNull(data.wouldPassSoftRegime, signalDebug.wouldPassSoftRegime, signal.wouldPassSoftRegime)),
    softRegimeRiskMultiplier: finiteOrNull(valueOrNull(data.softRegimeRiskMultiplier, signalDebug.softRegimeRiskMultiplier, signal.softRegimeRiskMultiplier)),
    requiredConfidenceUsed: finiteOrNull(valueOrNull(data.requiredConfidenceUsed, signalDebug.requiredConfidenceUsed, signal.requiredConfidenceUsed, signal.setupQuality?.minSetupConfidenceScore)),
    confirmationDisplacementAtr: finiteOrNull(valueOrNull(data.confirmationDisplacementAtr, signalDebug.confirmationDisplacementAtr)),
    oppositeWickPct: finiteOrNull(valueOrNull(data.oppositeWickPct, signalDebug.oppositeWickPct)),
    entryExtensionAtr: finiteOrNull(valueOrNull(data.entryExtensionAtr, signalDebug.entryExtensionAtr, signal.entryExtensionAtr)),
    recoveryModeActive: boolOrNull(valueOrNull(data.recoveryModeActive, signalDebug.recoveryModeActive, signal.recoveryModeActive)),
    pf5Telemetry: finiteOrNull(valueOrNull(data.pf5Telemetry, signalDebug.pf5Telemetry, signal.pf5Telemetry)),
    pf10Telemetry: finiteOrNull(valueOrNull(data.pf10Telemetry, signalDebug.pf10Telemetry, signal.pf10Telemetry)),
    expectancy5R: finiteOrNull(valueOrNull(data.expectancy5R, signalDebug.expectancy5R, signal.expectancy5R)),
  };
}

function resolvedTradeAuditSource(log) {
  return log.exitAudit && typeof log.exitAudit === 'object'
    ? log.exitAudit
    : log.audit && typeof log.audit === 'object'
      ? log.audit
      : log.tradeAudit && typeof log.tradeAudit === 'object'
        ? log.tradeAudit
        : null;
}

function buildLoggerExportFields(log = {}) {
  const tradeAudit = resolvedTradeAuditSource(log);
  return {
    blockedSetupMfe1hR: finiteOrNull(log.blockedSetupMfe1hR),
    blockedSetupMae1hR: finiteOrNull(log.blockedSetupMae1hR),
    blockedSetupMfe3hR: finiteOrNull(log.blockedSetupMfe3hR),
    blockedSetupMae3hR: finiteOrNull(log.blockedSetupMae3hR),
    takenTradeMfeR: finiteOrNull(log.takenTradeMfeR ?? tradeAudit?.mfeR),
    takenTradeMaeR: finiteOrNull(log.takenTradeMaeR ?? tradeAudit?.maeR),
    reached1R: boolOrNull(log.reached1R ?? tradeAudit?.reached1R),
    reached1_5R: boolOrNull(log.reached1_5R ?? tradeAudit?.reached1_5R),
    reached2R: boolOrNull(log.reached2R ?? tradeAudit?.reached2R),
    reached2_5R: boolOrNull(log.reached2_5R ?? tradeAudit?.reached2_5R),
  };
}

export function normalizeLogDiagnostics(log) {
  if (!log || typeof log !== 'object' || Array.isArray(log)) return log;
  const v2Missing = V2_DIAGNOSTIC_FIELDS.some(field => log[field] === undefined);
  const exportMissing = LOGGER_EXPORT_FIELDS.some(field => log[field] === undefined);
  if (!v2Missing && !exportMissing) return log;

  const loggedAt = new Date(log.time);
  const now = Number.isNaN(loggedAt.getTime()) ? new Date() : loggedAt;
  return {
    ...log,
    ...(v2Missing ? buildV2Diagnostics(log, log.marketRegime ?? null, now) : {}),
    ...(exportMissing ? buildLoggerExportFields(log) : {}),
  };
}
...
function resolveSetupDirection(data = {}, blockedSetup = null) {
  const candidates = [
    data.direction,
    data.signal?.action,
    data.signalDebug?.dbgAction,
    data.signalDebug?.nearSignalDirection,
    blockedSetup?.direction,
    blockedSetup?.blockedSetupDirection,
  ];
  for (const candidate of candidates) {
    const direction = normalizeTradeDirection(candidate);
    if (direction) return direction;
  }
  return null;
}

function resolveSetupConfidenceScore(data = {}, blockedSetup = null) {
  return firstFiniteOrNull(
    data.setupConfidenceScore,
    data.signal?.setupConfidenceScore,
    data.signal?.setupConfidence?.score,
    data.signalDebug?.setupConfidenceScore,
    data.signalDebug?.dbgSetupConfidenceScore,
    data.signalDebug?.dbgScore,
    blockedSetup?.setupConfidenceScore
  );
}

function resolveRawSetupConfidenceScore(data = {}, blockedSetup = null) {
  return firstFiniteOrNull(
    data.rawSetupConfidenceScore,
    data.confidenceRaw,
    data.signal?.setupConfidence?.rawScore,
    data.signalDebug?.rawSetupConfidenceScore,
    data.signalDebug?.confidenceRaw,
    data.signalDebug?.dbgRawSetupConfidenceScore,
    blockedSetup?.rawSetupConfidenceScore,
    resolveSetupConfidenceScore(data, blockedSetup)
  );
}

function resolveInitialRewardRisk(data = {}, blockedSetup = null) {
  return firstFiniteOrNull(
    data.initialRewardRisk,
    data.signal?.initialRewardRisk,
    data.signal?.setupQuality?.initialRewardRisk,
    data.signalDebug?.initialRewardRisk,
    data.signalDebug?.dbgInitialRewardRisk,
    data.signalDebug?.rrCandidate,
    blockedSetup?.initialRewardRisk
  );
}

function resolveSetupPassTelemetry(data = {}, setupConfidenceScore = null, initialRewardRisk = null, blockedSetup = null) {
  return {
    confidencePass: firstBoolOrNull(
      data.confidencePass,
      data.signalDebug?.confidencePass,
      data.signal?.confidencePass,
      data.signal?.setupQuality?.confidenceOk,
      blockedSetup?.confidencePass,
      setupConfidenceScore !== null ? setupConfidenceScore >= SETUP_CONFIDENCE_MIN_V2 : null
    ),
    rrPass: firstBoolOrNull(
      data.rrPass,
      data.signalDebug?.rrPass,
      data.signal?.rrPass,
      data.signal?.setupQuality?.rewardOk,
      blockedSetup?.rrPass,
      initialRewardRisk !== null ? initialRewardRisk >= MIN_RR_V2 : null
    ),
  };
}
...
function buildBlockedSetupFromLog(data, now) {
  ...
  const takeProfit = finiteOrNull(data.signal?.takeProfit);
  if (!direction || entry === null) return null;
  const setupConfidenceScore = resolveSetupConfidenceScore(data);
  const rawSetupConfidenceScore = resolveRawSetupConfidenceScore(data);
  const initialRewardRisk = resolveInitialRewardRisk(data);
  const passTelemetry = resolveSetupPassTelemetry(data, setupConfidenceScore, initialRewardRisk);
  ...
  return {
    ...
    rawSetupConfidenceScore,
    initialRewardRisk,
    entryPrice: entry,
    stopLoss,
    takeProfit,
    direction,
    marketRegime: data.marketRegime ?? data.signalDebug?.marketRegime ?? data.indicators?.marketRegime ?? null,
    trend1h: data.indicators?.trend1h ?? null,
    confidencePass: passTelemetry.confidencePass,
    rrPass: passTelemetry.rrPass,
...
function tradeOutcomeLogFields(result = {}, tradeAudit = null) {
  const audit = result?.exitAudit ?? result?.audit ?? tradeAudit ?? null;
  return {
    takenTradeMfeR: finiteOrNull(audit?.mfeR),
    takenTradeMaeR: finiteOrNull(audit?.maeR),
    reached1R: boolOrNull(audit?.reached1R),
    reached1_5R: boolOrNull(audit?.reached1_5R),
    reached2R: boolOrNull(audit?.reached2R),
    reached2_5R: boolOrNull(audit?.reached2_5R),
  };
}
...
async function persistAnalyticsEvents(redis, log) {
  ...
  if (isSetupBlockedByRiskOrKillSwitch(log)) {
    auditEvents.push({
      ...
      rawSetupConfidenceScore: finiteOrNull(log.rawSetupConfidenceScore ?? log.dbgRawSetupConfidenceScore),
      initialRewardRisk: finiteOrNull(log.initialRewardRisk),
      entryPrice: finiteOrNull(log.entryPrice),
      stopLoss: finiteOrNull(log.stopLoss),
      takeProfit: finiteOrNull(log.takeProfit),
      direction: normalizeTradeDirection(log.direction ?? log.signalDetected ?? log.dbgAction),
      confidencePass: boolOrNull(log.confidencePass),
      rrPass: boolOrNull(log.rrPass),
      blockedSetupReason: log.blockedSetupReason ?? null,
      blockedSetupMfe1hR: finiteOrNull(log.blockedSetupMfe1hR),
      blockedSetupMae1hR: finiteOrNull(log.blockedSetupMae1hR),
      blockedSetupMfe3hR: finiteOrNull(log.blockedSetupMfe3hR),
      blockedSetupMae3hR: finiteOrNull(log.blockedSetupMae3hR),
...
export async function saveLog(data) {
  try {
    ...
    const currentBlockedSetup = Number(blockedSetup?.blockedSetupTimeMs) === now.getTime()
      ? blockedSetup
      : null;
    const setupConfidenceScore = resolveSetupConfidenceScore(data, currentBlockedSetup);
    const rawSetupConfidenceScore = resolveRawSetupConfidenceScore(data, currentBlockedSetup);
    const initialRewardRisk = resolveInitialRewardRisk(data, currentBlockedSetup);
    const setupDirection = resolveSetupDirection(data, currentBlockedSetup);
    const resolvedEntryPrice = firstFiniteOrNull(data.result?.entry, data.signal?.entryPrice, currentBlockedSetup?.entryPrice, currentBlockedSetup?.blockedSetupPrice);
    const resolvedStopLoss = firstFiniteOrNull(data.result?.stopLoss, data.signal?.stopLoss, currentBlockedSetup?.stopLoss);
    const resolvedTakeProfit = firstFiniteOrNull(data.result?.takeProfit, data.signal?.takeProfit, currentBlockedSetup?.takeProfit);
    const tradeOutcomeFields = tradeOutcomeLogFields(data.result, data.tradeAudit);
    ...
    const log = {
      ...
      score:      firstFiniteOrNull(data.signal?.score, data.signalDebug?.dbgScore, setupConfidenceScore),
      setupConfidenceScore,
      rawSetupConfidenceScore,
      ...
      dbgEmaExpansionMissing: data.signalDebug?.dbgEmaExpansionMissing ?? data.signalDebug?.emaExpansionMissing ?? data.signal?.emaExpansionMissing ?? null,
      dbgEmaExpansionPenalty: data.signalDebug?.dbgEmaExpansionPenalty ?? data.signalDebug?.emaExpansionPenalty ?? data.signal?.emaExpansionPenalty ?? null,
      dbgEmaExpansionHandledAs: data.signalDebug?.dbgEmaExpansionHandledAs ?? data.signalDebug?.emaExpansionHandledAs ?? data.signal?.emaExpansionHandledAs ?? null,
    ...
    await redis.rpush(CYCLE_LOG_PRIMARY_KEY, JSON.stringify(log));
    // Keep enough recent cycle logs for multi-day audits without changing trading behavior.
    const len = await redis.llen(CYCLE_LOG_PRIMARY_KEY);

    if (len > CYCLE_LOG_RETENTION_LIMIT) {
      await redis.ltrim(CYCLE_LOG_PRIMARY_KEY, len - CYCLE_LOG_RETENTION_LIMIT, -1);
    }
...
export async function getLogs(limit = 0) {
  const result = await getLogsWithDebug(limit);
  return result.logs;
}

export async function getLogsWithDebug(limit = 0, options = {}) {
  try {
    const redis = getRedis();
    const preferredKey = typeof options?.preferredKey === 'string' && options.preferredKey.trim()
      ? options.preferredKey.trim()
      : null;
    const keysToTry = Array.from(new Set([
      ...(preferredKey ? [preferredKey] : []),
      CYCLE_LOG_PRIMARY_KEY,
      ...CYCLE_LOG_FALLBACK_KEYS,
    ].filter(Boolean)));
    const startIdx = limit > 0 ? -limit : 0;
    let raw = [];
    let keyUsed = keysToTry[0] || CYCLE_LOG_PRIMARY_KEY;
    let foundNonEmptyLogs = false;

    for (const key of keysToTry) {
      keyUsed = key;
      const candidate = await redis.lrange(key, startIdx, -1);
      if (!Array.isArray(candidate)) continue;
      if (candidate.length > 0) {
        raw = candidate;
        foundNonEmptyLogs = true;
        break;
      }
      raw = candidate;
    }

    if (!Array.isArray(raw) || (!foundNonEmptyLogs && raw.length === 0)) {
      return {
        logs: [],
        keyUsed,
        count: 0,
    ...
    console.error('getLogsWithDebug error:', err.message);
    return {
      logs: [],
      keyUsed: options?.preferredKey || CYCLE_LOG_PRIMARY_KEY,
      count: 0,
      latest: null,
