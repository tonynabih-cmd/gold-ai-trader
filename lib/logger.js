...
import { EXPECTANCY_KILL_POLICY, MIN_RR_V2, PF5_KILL_THRESHOLD, SETUP_CONFIDENCE_MIN_V2 } from './risk.js';
...
export function buildKillSwitchDiagnostics(botState = {}, signal = {}, now = new Date()) {
  const expectancyKill = botState?.expectancyKillSwitch && typeof botState.expectancyKillSwitch === 'object'
    ...
    : {};
  const activatedAtMs = Number(expectancyKill.activatedAt);
  const activatedAtIsValid = Number.isFinite(activatedAtMs) && activatedAtMs > 0;
  const killSwitchActivatedAt = activatedAtIsValid ? new Date(activatedAtMs).toISOString() : null;
  const hoursSinceActivation = expectancyKill.active === true && activatedAtIsValid
    ? Number(((now.getTime() - activatedAtMs) / (60 * 60 * 1000)).toFixed(4))
    : null;
  const pfValueUsed = firstFiniteOrNull(signal?.pf5Telemetry, botState?.rollingProfitFactor5);

  return {
    killSwitchActive: expectancyKill.active === true,
    killSwitchActivatedAt,
    currentTimeUTC: now.toISOString(),
    hoursSinceActivation,
    resetReason: expectancyKill.resetReason ?? null,
    pfValueUsed,
    pfThresholdUsed: PF5_KILL_THRESHOLD,
    killSwitchPolicy: EXPECTANCY_KILL_POLICY,
  };
}
...
export async function saveLog(data) {
  try {
    ...
    const killSwitchDiagnostics = buildKillSwitchDiagnostics(data.botState, data.signal, now);
