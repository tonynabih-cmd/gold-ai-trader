...
export async function saveAudit(data) {
  try {
    ...
    await redis.rpush(DAILY_AUDIT_HISTORY_KEY, JSON.stringify(summary));
    const len = await redis.llen(DAILY_AUDIT_HISTORY_KEY);
    if (len > DAILY_AUDIT_HISTORY_CAP) {
      await redis.ltrim(DAILY_AUDIT_HISTORY_KEY, len - DAILY_AUDIT_HISTORY_CAP, -1);
    }
    return true;
  } catch (err) {
