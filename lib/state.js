...
export async function renewCandleLock(lockHandle, ttlSeconds = 120) {
  try {
    if (!lockHandle?.key || !lockHandle?.token) return false;
    const owned = await verifyCandleLockOwnership(lockHandle);
    if (!owned) return false;

    const result = await redis.set(lockHandle.key, lockHandle.token, { xx: true, ex: ttlSeconds });
    if (result === 'OK') {
...
export async function releaseCandleLock(lockHandle) {
  try {
    ...
    const owned = await verifyCandleLockOwnership(lockHandle);
    if (!owned) return false;
    await redis.del(lockHandle.key);
    return true;
  } catch (err) {
