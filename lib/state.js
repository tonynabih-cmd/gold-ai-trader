...
const DAILY_AUDIT_HISTORY_CAP = 2000;
export const BOT_STATE_KEY = 'bot_state';
...
export const CANDLE_LOCK_TTL_SECONDS = 75;
...
export async function acquireCandleLock(candleTime, ownerToken) {
  try {
    ...
    const ttlSeconds = CANDLE_LOCK_TTL_SECONDS;
    const result = await redis.set(key, token, { nx: true, ex: ttlSeconds });
    
    if (result === 'OK') {
      console.log(`[LOCK] ✓ Acquired lock for candle ${candleTime} (owner=${token}, ttl=${ttlSeconds}s)`);
      return {
        key,
        token,
        candleTime,
      ...
      console.warn(`[LOCK] ✗ Failed to acquire lock for candle ${candleTime} — another invocation is processing it or a prior lock is still active (ttl=${ttlSeconds}s)`);
      return null;
    }
  } catch (err) {
...
const DEFAULT_STATE = {
  ...
  lastRiskSyncAt:          0,
  recentOutcomes:          [],
  rollingWinRate10:        null,
  rollingProfitFactor15:   null,
  rollingProfitFactor5:    null,
  rollingGrossProfit15:    0,
  rollingGrossLoss15:      0,
  performanceReviewNeeded: false,
  ...
  chartUpdatedThisCycle:   false,
  schedulerSource:         'unknown',
  lastMarketClosedLogAt:   0,
  lastMarketClosedReason:  '',
  dashboardChart:          [],
};

export async function loadState() {
  try {
    const saved = await redis.get(BOT_STATE_KEY);
    if (!saved) {
    ...
    const state = {
      ...
      recentOrderTimestamps: Array.isArray(saved.recentOrderTimestamps)
        ? saved.recentOrderTimestamps
            .map(ts => Number(ts))
      ...
      rollingWinRate10:    saved.rollingWinRate10 == null ? null : parseFloat(saved.rollingWinRate10),
      rollingProfitFactor15: saved.rollingProfitFactor15 == null ? null : parseFloat(saved.rollingProfitFactor15),
      rollingProfitFactor5: saved.rollingProfitFactor5 == null ? null : parseFloat(saved.rollingProfitFactor5),
      rollingGrossProfit15: parseFloat(saved.rollingGrossProfit15 ?? 0),
      rollingGrossLoss15:  parseFloat(saved.rollingGrossLoss15 ?? 0),
      performanceReviewNeeded: saved.performanceReviewNeeded === true,
      performanceReviewReason: typeof saved.performanceReviewReason === 'string' ? saved.performanceReviewReason : '',
      directionalLossCircuit: saved.directionalLossCircuit && typeof saved.directionalLossCircuit === 'object'
        ? {
            ...DEFAULT_STATE.directionalLossCircuit,
            ...saved.directionalLossCircuit,
          }
        : { ...DEFAULT_STATE.directionalLossCircuit },
      expectancyKillSwitch: saved.expectancyKillSwitch && typeof saved.expectancyKillSwitch === 'object'
        ? {
            ...DEFAULT_STATE.expectancyKillSwitch,
            ...saved.expectancyKillSwitch,
          }
        : { ...DEFAULT_STATE.expectancyKillSwitch },
      ...
      lastDisabledAlert:   parseInt(saved.lastDisabledAlert ?? 0),
      lastCriticalAlert:   parseInt(saved.lastCriticalAlert ?? 0),
      currentCycleTime:    parseInt(saved.currentCycleTime ?? 0),
      currentCycleReason:  typeof saved.currentCycleReason === 'string' ? saved.currentCycleReason : '',
      lastValidDataTime:   parseInt(saved.lastValidDataTime ?? 0),
      dataFreshnessStatus: typeof saved.dataFreshnessStatus === 'string' ? saved.dataFreshnessStatus : 'UNKNOWN',
      chartUpdatedThisCycle: saved.chartUpdatedThisCycle === true,
      schedulerSource:     typeof saved.schedulerSource === 'string' ? saved.schedulerSource : 'unknown',
      lastMarketClosedLogAt: parseInt(saved.lastMarketClosedLogAt ?? 0),
      lastMarketClosedReason: typeof saved.lastMarketClosedReason === 'string' ? saved.lastMarketClosedReason : '',
      dashboardChart:      Array.isArray(saved.dashboardChart) ? saved.dashboardChart : [],
...
export async function saveStateWithOptions(botState, options = null) {
  try {
    ...
    const { candles5m: discardedCandles5m, ...baseStateToSave } = botState;
    void discardedCandles5m;
    ...
    await redis.set(BOT_STATE_KEY, stateToSave);
    botState.stateVersion = nextVersion;
    botState.stateUpdatedAt = stateToSave.stateUpdatedAt;
    return true;
  } catch (err) {
...
export async function saveAudit(data) {
  try {
    const summary = {
      eventType: 'daily_audit_summary',
      ts: new Date().toISOString(),
      date: data?.date ?? new Date().toISOString().slice(0, 10),
      report: typeof data?.report === 'string' ? data.report : null,
      strategyVersion: data?.strategyVersion ?? STRATEGY_VERSION,
      schedulerSource: data?.schedulerSource ?? 'unknown',
      totalDecisions: data?.totalDecisions ?? data?.totalCycles ?? null,
      tradesExecuted: data?.tradesExecuted ?? data?.trades ?? null,
      setups: data?.setups ?? null,
      totalRejects: data?.totalRejects ?? null,
      brokerErrors: data?.brokerErrors ?? null,
      profitFactor: data?.profitFactor ?? data?.pfStr ?? null,
      cvar95: data?.cvar95 ?? null,
      worstLoss: data?.worstLoss ?? null,
      averageLoss: data?.averageLoss ?? null,
      lossCount: data?.lossCount ?? null,
    ...
    const existingRaw = await redis.lrange(DAILY_AUDIT_HISTORY_KEY, 0, -1).catch(() => []);
    const deduped = (Array.isArray(existingRaw) ? existingRaw : []).filter(entry => {
      try {
        const parsed = typeof entry === 'string' ? JSON.parse(entry) : entry;
        return parsed?.date !== summary.date;
      } catch {
        return true;
      }
    });
    const nextHistory = [...deduped, JSON.stringify(summary)].slice(-DAILY_AUDIT_HISTORY_CAP);

    await redis.del(DAILY_AUDIT_HISTORY_KEY);
    for (const entry of nextHistory) {
      await redis.rpush(DAILY_AUDIT_HISTORY_KEY, typeof entry === 'string' ? entry : JSON.stringify(entry));
    }
    return true;
  } catch (err) {
