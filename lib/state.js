...
export async function saveBadFillResponse(data) {
  try {
    await redis.set('lastBadFillResponse', JSON.stringify(data));
    return true;
  } catch (err) {
    console.error('[STATE] Failed to save bad fill response:', err.message);
    return false;
  }
}
