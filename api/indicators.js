export function calculateIndicators(candles5m, candles1h) {
  try {
    ...
    if (Math.abs(slopePercent) < 0.02) {
      return { skip: true, reason: 'SKIP: Weak trend - sideways market' };
    }
    ...
    if (currentATR > atrAverage * 2) {
      return { skip: true, reason: 'SKIP: Abnormal volatility - news event' };
    }
    if (currentATR < 1 || currentATR > 8) {
      return { skip: true, reason: 'SKIP: ATR out of normal range' };
    }
